...
2. **Trigger 매칭 시 강제 읽기** — 편집 path 가 `## Trigger → Read` 의 `when` 패턴과 매칭하면 그 항목의 문서 먼저.
...
- `## Required Context` / `## On-Demand Context` / `## Trigger → Read` = **path 메타만** prefix 에 박힘. 본문은 lazy — 필요 시점에 도구로 읽기.
...
- `## On-Demand Context` — 필요 시 자율 참조 후보 (lazy)
- `## Trigger → Read` — 특정 작업 패턴 매칭 시 강제 읽기
